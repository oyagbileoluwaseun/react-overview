...
function DataFetch3() {
  ...
  const requestFn = async (signal: AbortSignal) => {
    if (search.trim()) {
      return await searchProducts(search, signal);
    }
    return await fetchProductsByCategory(category, signal);
  };

  const { data, loading, error } = useFetch(requestFn, [category, search]);
